...
### 1. Apify (U3 — inspiration mining)
- Create an Apify account → Console → Integrations → copy the **API token**.
- Add Supabase secret (dashboard → Edge Functions → Secrets, or CLI):
  `APIFY_TOKEN=<token>`
- Then I build `content-scrape` (uses `instagram-reel-scraper` + discovery) + the daily cron. Budget ~$1.50–2.60 / 1,000 reels — a per-run cap will be set.
...
| Secret | For | Status |
|---|---|---|
| `APIFY_TOKEN` | U3 scrape | needed |
| `POSTIZ_API_KEY` | U8 publish | needed |
| `POSTIZ_BASE_URL` | U8 publish | needed |
| `HIGGSFIELD_API_KEY` | optional REST generation | optional |
