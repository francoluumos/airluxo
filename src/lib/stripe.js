...
export async function createPaymentIntent({ listingId, rateId, quantity, crossBorder, delivery, startDate, endDate, pickupTime, returnTime, promoCode }) {
  const { data, error } = await supabase.functions.invoke('stripe-create-payment', {
    body: { listing_id: listingId, rate_id: rateId, quantity, cross_border: crossBorder, delivery, start_date: startDate, end_date: endDate, pickup_time: pickupTime, return_time: returnTime, promo_code: promoCode },
  });
