...
export const DOCS = {
  ...
  admin: {
    ...
    changelog: [
      { date: '2026-06-21', version: 'Partner dashboard logo + car-detail dark-theme fix', items: ['The partner dashboard sidebar now shows the partner’s own brand logo (from their brand kit) instead of the AIRLUXO wordmark, so it feels like their portal. Also fixed the car-detail hero on dark partner themes: the photo scrim is now always dark with light text (it was inverting to a light wash that hid the car name).'] },
      { date: '2026-06-21', version: 'Partner site — FAQ section', items: ['Add a partner-authored FAQ to the white-label site: build Q&A pairs in Brand & pitch → Website → Layout → FAQ (in the partner’s language) and they render as an accordion before the footer. Saved on publish/republish. The AIRLUXO partner-acquisition landing also gained a bilingual (DE/EN) FAQ as a bottom section.'] },
      { date: '2026-06-21', version: 'Partner site — hero eyebrow + dark-theme hover fix', items: ['The hero now shows an editable gold kicker above the headline (Brand & pitch → Layout → “Hero eyebrow”, e.g. “Drive your dream”) instead of repeating the company name. Fixed a dark-theme bug where buttons (e.g. the hero Search) lost their label on hover — the hover shade now tracks the button, not the page background. Brand-strip logos also load a touch smoother (eager + reserved width).'] },
      { date: '2026-06-21', version: 'Partner site — centered-hero background media', items: ['When the hero layout is Centered you can now set a full-bleed background — an image or a video (autoplay, muted, looped) — in Brand & pitch → Website → Layout → Hero background (upload or paste a URL, stored in brand-assets/<id>/hero/). A dark scrim is added automatically so the headline stays legible. The partner-site header CTA also now uses the brand-gold pill (matching the “Book now” preview button).'] },
      { date: '2026-06-20', version: 'Partner cars — organize photos into storage', items: ['Brand & pitch → Cars & images gets “Organize photos → storage”: it mirrors a partner’s scraped car pictures from their external (Wix etc.) URLs into our own bucket, under a tidy per-car folder brand-assets/<partner>/cars/<listing>/, and repoints each listing at the stored copies. Idempotent (already-mirrored photos are skipped); served from our domain instead of depending on the source site.'] },
      { date: '2026-06-20', version: 'Partner site — brand strip as logos', items: ['Brand & pitch → Website → Layout can now switch the scrolling brand strip from text names to the partner’s own brand logos (same strip height). Build the logo set three ways: pick from the images the ingest already scraped, paste a logo URL, or upload files — stored in the partner’s brand-assets/<id>/brand-logos/ folder. Saved on publish/republish.'] },
      { date: '2026-06-20', version: 'Partner site — dark-theme fix + fleet shows', items: ['Themed partner sites with a dark brand background now render correctly: the brand kit derives a full neutral ramp (surfaces, borders, muted text) from the partner’s background↔text pair instead of leaving AIRLUXO’s light defaults, so no more white-on-white boxes, harsh white divider lines or invisible labels. The white-label header is rebalanced (logo left, a single fleet CTA right) and the hero no longer shows AIRLUXO marketplace copy as a fallback. The published site’s fleet now lists the partner’s cars even while they’re still a pipeline prospect (security-definer read gated on a published site).'] },
      { date: '2026-06-20', version: 'Partner site — per-partner layout', items: ['Brand & pitch → Website gets a Layout panel to tune one partner’s site without affecting any other partner or the marketplace: toggle the hero stats row (marketplace counts), the brand strip and the fleet map, and pick a hero layout (split with image · centered). Saved on publish/republish; the flags live in the site config and the shared home reads them only for that partner.'] },
      { date: '2026-06-18', version: 'Developer — Cron & jobs observability', items: ['Developer gets a Cron & background jobs panel: every scheduled pg_cron job (schedule + on/off), recent run history (status, message, duration), and the cron→edge-function HTTP responses — which surfaces silent failures where a cron reports “succeeded” but its function actually returned 401/500 (e.g. a missing Vault key). Plus recent partner-ingest jobs with errors. Refresh on demand.'] },
      { date: '2026-06-17', version: 'Ingest — structured car table', items: ['“Analyze website” now also AI-extracts a structured car list from the fleet page (make · model · year · CHF/day · power · seats · gearbox · fuel · photo). The review shows it as an editable table — fix any field, select rows, and “Create selected” bulk-builds the listings (photo = card image), flowing into the storefront preview, the website and the partner dashboard fleet. Image grid also filters out brand logos and Wix lazy-load/blur placeholders. Re-analyze to refresh.'] },
      { date: '2026-06-17', version: 'Partner site hero = their USP + staging subdomains', items: ['The partner site’s hero now shows the partner’s name + USP/tagline (from the extracted copy) instead of the AIRLUXO marketing headline. Admin/partner host detection now also resolves the per-environment subdomains staging.admin.airluxo.ch + staging.partner.airluxo.ch (used for staging; the bare admin./partner. are for production).'] },
      { date: '2026-06-17', version: 'Partner website = themed AIRLUXO home', items: ['The published partner site is now an exact copy of the AIRLUXO homepage — the same hero, search, filters, fleet grid, car detail and booking flow — scoped to the partner’s cars and re-skinned with their brand kit (colours, fonts, logo) + their legal pages, with AIRLUXO’s host-recruitment sections hidden. It runs the real Home component (one codebase → guaranteed parity), at airluxo.ch/p/&lt;slug&gt; and on the partner’s own domain.'] },
      { date: '2026-06-17', version: 'Brand & pitch — build the fleet from scraped photos', items: ['The review is now full-width and you can build a prospect’s cars right there: select the scraped images, tag one “Hero”, and “New car from selected” captures the car details (make/model/year/price/power/seats/gearbox/fuel/colours/km/city/description) and creates the listing with that gallery — no impersonation needed. Existing cars are shown with thumbnails + photo counts, and selected images can also be attached to an existing car. New cars flow straight into the storefront preview and the website listings. The token sales-preview is capped at 5 cars.'] },
      { date: '2026-06-17', version: 'Pipeline — leads open as full views', items: ['Opening a lead (gear or its name) now takes over the dashboard area next to the menu instead of a popup, with breadcrumbs (Pipeline / Company) to step back. The Brand & pitch review is a further breadcrumb level (Pipeline / Company / Brand & pitch) — more room for the brand kit, car images, legal, publish and domain panels.'] },
