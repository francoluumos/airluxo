...
export function AuthProvider({ children }) {
  ...
  const loadProfiles = useCallback(async (user) => {
    ...
    // Newsletter consent is keyed by email in newsletter_subscribers from the first
    // touch (footer / checkout), so no opt-in adoption is needed on sign-up; we only
    // link the customer_id, done lazily by the edge function on the next subscribe.
